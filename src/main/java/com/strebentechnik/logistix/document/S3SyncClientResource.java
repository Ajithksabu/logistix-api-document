...
public class S3SyncClientResource extends CommonResource {
    ...
    public Response uploadFile(@MultipartForm DocumentDto documentDto) throws Exception {

        if (documentDto.fileName == null || documentDto.fileName.isEmpty()) {
            return Response.status(Status.BAD_REQUEST).build();
        }

        if (documentDto.mimeType == null || documentDto.mimeType.isEmpty()) {
            return Response.status(Status.BAD_REQUEST).build();
        }

        PutObjectResponse putResponse = s3.putObject(buildPutRequest(documentDto),
                RequestBody.fromFile(uploadToTemp(documentDto.file)));
        if (putResponse != null) {
